...
, instanceId) {
  ...
  const macdData = rows
    ...
    .map((d) => ({ time: Number(d.time) + 19800, value: Number(d.macd) }));
  ...
  const series = indicatorSeriesRef.current?.[instanceId || "MACD"];
  if (series) {
  ...
  latestIndicatorValuesRef.current[instanceId || "MACD"] = {
